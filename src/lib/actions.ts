...
import { reanchor } from './annotations'
...
async function revalidateTrack(trackId: string) {
  const supabase = await createClient()
  ...
  await revalidateAlbum(data.album_id)
  revalidatePath(`/admin/tracks/${trackId}`)
  revalidateShared()
}

async function revalidateAlbum(albumId: string) {
  const supabase = await createClient()
  const { data } = await supabase
    .from('albums')
    .select('slug')
